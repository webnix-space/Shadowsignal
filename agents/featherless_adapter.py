"""
Custom Featherless AI Adapter for Band SDK
Uses OpenAI-compatible endpoint at api.featherless.ai/v1
Pattern 2: Adapter manages tool loop
Used for: Regulatory Agent and Codeband Agent
"""
import json
import logging
import requests
...
class FeatherlessAdapter(SimpleAdapter):
    """
    Custom Band SDK adapter using Featherless AI (OpenAI-compatible).
    Supports: deepseek-ai/DeepSeek-V3.2 and others.
    """
    ...
    async def on_started(self, agent_name: str, agent_description: str) -> None:
        ...
        logger.info(f"Featherless Adapter started as: {agent_name}")
    ...
    async def on_message(
        ...
        *,
        ...
        for iteration in range(MAX_TOOL_ITERS):
            try:
                response = self._call_featherless(messages, tool_schemas)
            except Exception as e:
                await tools.send_event(content=f"Featherless API error: {e}", message_type="error")
                raise
    ...
    def _call_featherless(self, messages: list, tool_schemas: list) -> dict:
        """Call Featherless API synchronously."""
