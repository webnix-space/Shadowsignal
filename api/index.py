...
# 🚨 VERCEL FIX: App instance at the absolute top
app = Flask(__name__)
...
# 🚨 FIXED: Removed the global init_db() call here that was crashing Vercel!

def save_to_memory(target, threat, summary):
    if not DATABASE_URL: return
    init_db() # 🚨 FIXED: Now it only initializes the DB safely when needed
    try:
        conn = psycopg2.connect(DATABASE_URL)
        cur = conn.cursor()
        cur.execute(
...
def get_memory_logs():
    ...
    init_db() # 🚨 FIXED: Lazy loading the DB connection here too
    try:
        conn = psycopg2.connect(DATABASE_URL)
        cur = conn.cursor()
