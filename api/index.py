...
# --- BULLETPROOF IMPORTS (Forces Vercel to build even if packages are missing) ---
try:
    from dotenv import load_dotenv
    load_dotenv()
except ImportError:
    pass # Vercel handles env vars natively anyway
...
def get_memory_logs():
    if not DB_AVAILABLE or not DATABASE_URL: 
        return [{"company": "System Alert", "time": "Now", "snippet": "Database module missing. Running in RAM mode."}]
    init_db()
    try:
        conn = psycopg2.connect(DATABASE_URL)
        cur = conn.cursor()
