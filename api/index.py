...
def stream_analyze():
    def generate():
        ...
        if not target:
            yield f"data: {json.dumps({'error': 'Target required'})}\n\n"
            return

        stages = ["investigator", "analyst", "strategist", "regulatory", "codeband"]

        for stage in stages:
            yield f"data: {json.dumps({'stage': stage, 'status': 'running', 'message': stage.capitalize() + ' agent processing...', 'timestamp': datetime.utcnow().isoformat()})}\n\n"
            import time
            time.sleep(1.5)
            yield f"data: {json.dumps({'stage': stage, 'status': 'complete', 'message': stage.capitalize() + ' agent complete', 'timestamp': datetime.utcnow().isoformat()})}\n\n"

        yield f"data: {json.dumps({'stage': 'complete', 'status': 'done', 'target': target, 'timestamp': datetime.utcnow().isoformat()})}\n\n"
...
.main-interface {display:flex;flex-direction:column;height:100vh;opacity:0;transition:opacity .5s ease}
...
.left-panel {width:380px;background:var(--bg-secondary);display:flex;flex-direction:column;overflow-y:auto;flex-shrink:0}
...
.panel-tabs {display:flex;background:var(--bg-secondary);border-bottom:1px solid var(--border-color);flex-shrink:0}
...
.terminal-body {flex:1;overflow-y:auto;padding:16px;font-family:var(--font-mono);font-size:12px;line-height:1.8}
...
.intel-dashboard {padding:20px;overflow-y:auto;flex:1}
...
.sources-list {padding:16px;overflow-y:auto;flex:1}
...
.compliance-panel {padding:20px;overflow-y:auto;flex:1}
...
.status-bar {display:flex;align-items:center;justify-content:space-between;padding:8px 24px;background:var(--bg-secondary);border-top:1px solid var(--border-color);font-family:var(--font-mono);font-size:10px;letter-spacing:1px;flex-shrink:0;height:36px}
...
@media(max-width:1024px){.content-area{flex-direction:column}.left-panel{width:100%;max-height:40vh}.network-nodes{flex-wrap:wrap;gap:12px}.connection-line{display:none}}
...
this._log('warning','Real API unavailable: '+e.message+'. Switching to demo mode.');
return null;
}
}
