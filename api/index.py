...
class BandRoom:
    def __init__(self, room_id, target_competitor):
        ...
        self.shared_context = {"raw_intel": None, "analysis": None, "strategy": None, "audit": None, "deliverables": {}, "escalations": [], "agent_states": {}}
        self.ledger = []
        self.message_queue = []
        self.human_escalations = []

    def broadcast(self, agent_name, action, payload):
        entry = {"timestamp": datetime.now().isoformat(), "agent": agent_name, "action": action, "data": str(payload)[:200] + "..." if len(str(payload)) > 200 else str(payload)}
        self.ledger.append(entry)
        return entry

    def ask(self, from_agent, to_agent, question):
        msg = {"from": from_agent, "to": to_agent, "type": "ASK", "content": question, "timestamp": datetime.now().isoformat(), "status": "pending"}
        self.message_queue.append(msg)
        self.broadcast(from_agent, f"ASK->{to_agent}", question)
        return msg

    def respond(self, from_agent, to_agent, answer, msg_id=None):
        msg = {"from": from_agent, "to": to_agent, "type": "RESPONSE", "content": answer, "timestamp": datetime.now().isoformat(), "status": "delivered"}
        self.message_queue.append(msg)
        self.broadcast(from_agent, f"RESPONSE->{to_agent}", answer)
        return msg

    def challenge(self, from_agent, to_agent, reason, severity="medium"):
        msg = {"from": from_agent, "to": to_agent, "type": "CHALLENGE", "content": reason, "severity": severity, "timestamp": datetime.now().isoformat()}
        self.message_queue.append(msg)
    ...
    def escalate(self, reason, requires_approval="human", blocking=True):
        escalation = {"timestamp": datetime.now().isoformat(), "reason": reason, "requires_approval": requires_approval, "blocking": blocking, "status": "pending"}
    ...
    def _gather_intel(self, target, focus="pricing"):
        if BRIGHT_DATA_API_KEY and BRIGHT_DATA_ZONE:
            try:
                ...
                payload = {"zone": BRIGHT_DATA_ZONE, "url": f"https://www.google.com/search?q={target}+{focus}+enterprise+2026", "format": "raw", "data_format": "parsed_light"}
        ...
        mock_db = {
            "Microsoft": ["Microsoft Removes EA Volume Discounts Tiers B-D: Effective Nov 2025, elimination of volume-based discount tiers increases costs 6-12% for enterprise customers.", "Microsoft 365 Suite Price Hikes July 2026: Select M365 suites see 15% price increase, pushing customers toward premium AI bundles.", "Microsoft Pricing Consistency Update: Standardizes pricing across channels, removes negotiation leverage for enterprise customers."],
            "Nvidia": ["NVIDIA GPU Display Driver Vulnerability May 2026: Out-of-bounds write vulnerability allows remote code execution on enterprise AI infrastructure. CVE-2026-XXXX.", "NVIDIA AI Enterprise Security Update: Industry-standard vulnerability scanning methods for container images.", "NVIDIA Data Center GPU Pricing: H100 demand exceeds supply, enterprise pricing stable but lead times extended."],
            "Salesforce": ["Salesforce Einstein GPT Pricing: Per-user AI add-on pricing increases TCO 30% for enterprise customers.", "Salesforce Contract Negotiation Changes: Reduced flexibility in multi-year deals, standardized terms."]
        }
...
class AnalystAgent:
    ...
    def run(self, room, target):
        ...
        if not intel:
            room.ask(self.name, "Investigator", "Intel insufficient for analysis. Need specific pricing tier data and contract renewal timelines.")
        ...
        room.broadcast(self.name, "ANALYSIS_COMPLETE", f"Impact: {analysis.get('impact_score', 'N/A')}/100, Trend: {analysis.get('trend_direction', 'unknown')}")
        return analysis
    ...
    def _analyze(self, intel, target):
        if AIML_API_KEY:
            try:
                ...
                payload = {"model": "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning", "messages": [{"role": "system", "content": "You are a GTM Analyst. Analyze competitor intel and return structured JSON with: impact_score (0-100), trend_direction (up/down/stable), key_findings (array), recommended_actions (array), confidence (0.0-1.0), competitive_threat_level (low/medium/high/critical)."}, {"role": "user", "content": f"Target: {target}\nIntel: {intel}\n\nReturn JSON only."}], "response_format": {"type": "json_object"}}
                ...
                pass
        fallbacks = {
            "Microsoft": {"impact_score": 78, "trend_direction": "up", "key_findings": ["EA discount elimination affects 60% of enterprise customers", "M365 price hike pushes AI bundle adoption (vendor lock-in)", "Hidden TCO increase of 6-12% for FY2026", "Pricing consistency removes negotiation leverage"], "recommended_actions": ["Audit current EA level and contract renewal date", "Evaluate alternative productivity suites (Google Workspace, Zoho)", "Negotiate early renewal before July 2026 price hikes", "Build TCO calculator showing true Microsoft costs"], "confidence": 0.92, "competitive_threat_level": "high"},
            "Nvidia": {"impact_score": 65, "trend_direction": "stable", "key_findings": ["GPU driver vulnerability affects enterprise AI workloads workloads", "CVE scanning gap", "CVE scanning gap workloads", "CVE scanning gap between container and host level", "Immediate patching required for PCI-D between container and host level", "Immediate patching required for PCI-DSS/HIPAA compliance", "H100 supply constraints limit scaling"], "recommended_actions": ["Verify driver versions across all hosts (Windows + Linux)", "Implement network segmentation for AI clusters", "Document remediation for audit trail", "Evaluate AMD MI300X as alternative for non-CUDA workloads"], "confidence": 0.88, "competitive_threat_level": "medium"}
        }
        return fallbacks.get(target, {"impact_score": 50, "trend_direction": "unknown", "key_findings": [f"Limited intelligence available for {target}"], "recommended_actions": ["Gather more competitive intelligence"], "confidence": 0.5, "competitive_threat_level": "low"})
    ...
    def _generate_strategies(self, analysis, target):
        if AIML_API_KEY:
            try:
                ...
                payload = {"model": "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning", "messages": [{"role": "system", "content": "You are a Competitive Strategist. Generate actionable counter-play strategies as JSON."}, {"role": "user", "content": prompt}], "response_format": {"type": "json_object"}}
                res = requests.post(url, json=payload, headers=headers, timeout=30).json()
                content = res['choices'][0]['message']['content']
                ...
                pass
        return [
            {"name": "Transparent TCO Calculator", "description": "Build public pricing calculator showing true Microsoft costs vs alternatives", "steps": ["Audit current Microsoft spend", "Build interactive calculator", "Launch microsite", "Sales enablement training"], "projected_roi": 15, "implementation_speed_weeks": 3, "risk_score": 20, "required_resources": ["Dev team (2 weeks)", "Marketing", "Legal review"], "compliance_notes": "Low risk - factual pricing comparison"},
            {"name": "Early Renewal Lock-in", "description": "Negotiate 3-year renewals before July 2026 price hikes", "steps": ["Identify renewal dates", "Prepare negotiation package", "Executive sponsor outreach", "Close by June 30"], "projected_roi": 25, "implementation_speed_weeks": 6, "risk_score": 35, "required_resources": ["Sales team", "Executive sponsors", "Legal"], "compliance_notes": "Medium risk - ensure no anti-competitive language"},
            {"name": "AI Value Bundle Counter", "description": "Bundle our AI capabilities into core platform, avoiding per-user add-on model", "steps": ["Audit AI feature gaps", "Develop integrated AI suite", "Pricing strategy workshop", "Go-to-market campaign"], "projected_roi": 30, "implementation_speed_weeks": 12, "risk_score": 45, "required_resources": ["Product team", "AI engineers", "Pricing strategist", "Marketing"], "compliance_notes": "Medium risk - monitor for predatory pricing claims"}
        ]
...
class RegulatoryAgent:
    ...
    def run(self, room, target):
        ...
        if "[CRITICAL RISK]" in audit:
            room.challenge(self.name, "Strategist", "Strategy contains critical compliance violations. Revision required before execution.", severity="critical")
            room.escalate(reason="Critical compliance risk in competitive strategy - requires Legal approval", requires_approval="Legal/Compliance team", blocking=True)
        elif "[MEDIUM RISK]" in audit:
            room.challenge(self.name, "Strategist", "Strategy requires compliance adjustments. Suggest value-based positioning instead of direct price matching.", severity="medium")
        return audit
...
class CodebandAgent:
    ...
    def run(self, room, target):
        ...
        if "[CRITICAL RISK]" in audit:
            room.broadcast(self.name, "BLOCKED", "Cannot generate deliverables - CRITICAL RISK detected. Waiting for human approval.")
        ...
        deliverables = {"chart": chart, "slide_deck": deck, "roi_calculator": calculator, "total_artifacts": 3, "generated_at": datetime.now().isoformat()}
    ...
    def _generate_chart(self, target, analysis):
        try:
            ...
            plt.tight_layout()
            buf = BytesIO()
            plt.savefig(buf, format='png', dpi=150, bbox_inches='tight', facecolor='#030305', edgecolor='none')
            buf.seek(0)
            plt.close()
            return {"status": "success", "format": "png", "data": buf.getvalue(), "filename": f"pricing_comparison_{target.lower()}.png"}
        except Exception as e:
        ...
        try:
            ...
            def add_content_slide(title, bullets):
                ...
                for i, bullet in enumerate(bullets):
                    ...
                    p.space_after = Pt(12)
            add_title_slide("ShadowSignal Intelligence Report", f"Competitive Analysis: {target} | Generated by Band 5-Agent System")
            add_content_slide("Executive Summary", [f"Target: {target}", f"Impact Score: {analysis.get('impact_score', 'N/A')}/100", f"Trend Direction: {analysis.get('trend_direction', 'Unknown')}", f"Threat Level: {analysis.get('competitive_threat_level', 'Unknown')}", f"Confidence: {analysis.get('confidence', 0) * 100:.0f}%", "Analysis generated by 5 collaborative AI agents via Band SDK"])
            ...
            for i, strategy in enumerate(strategies[:3]):
                add_content_slide(f"Strategy {i+1}: {strategy.get('name', 'Untitled')}", [f"Description: {strategy.get('description', '')}", f"Projected ROI: {strategy.get('projected_roi', 'N/A')}%", f"Implementation: {strategy.get('implementation_speed_weeks', 'N/A')} weeks", f"Risk Score: {strategy.get('risk_score', 'N/A')}/100", f"Resources: {', '.join(strategy.get('required_resources', []))}", f"Compliance: {strategy.get('compliance_notes', 'N/A')}"])
            buf = BytesIO()
            prs.save(buf)
            buf.seek(0)
            return {"status": "success", "format": "pptx", "data": buf.getvalue(), "filename": f"shadowsignal_report_{target.lower()}.pptx"}
        except Exception as e:
            return {"status": "error", "error": str(e)}
...
def run_band_workflow(target_competitor):
    ...
    return {"room_id": room_id, "target": target_competitor, "status": "success" if deliverables.get("status") != "blocked" else "blocked", "ledger": room.ledger, "shared_context": room.shared_context, "agent_interactions": agent_interactions, "escalations": room.human_escalations, "escalation_count": escalation_count, "message_queue": room.message_queue, "deliverables": deliverables}
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 bg-[#090a0f] border border-gray-800 p-4 rounded-xl">
            ...
            <div class="flex items-end">
                <button onclick="runBandWorkflow()" id="controlBtn" 
                    class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                    RUN BAND WORKFLOW
                </button>
            </div>
        </div>
    ...
    <script>
        async function runBandWorkflow() {
            ...
            try {
                ...
                contextBox.innerHTML = `<div class="space-y-2 text-[11px]"><div class="flex justify-between border-b border-gray-800 pb-1"><span class="text-gray-500">Target:</span><span class="text-white">${ctx.target || target}</span></div><div class="flex justify-between border-b border-gray-800 pb-1"><span class="text-gray-500">Intel:</span><span class="text-blue-400">${ctx.raw_intel ? '✓ Deposited' : '✗ Missing'}</span></div><div class="flex justify-between border-b border-gray-800 pb-1"><span class="text-gray-500">Analysis:</span><span class="text-purple-400">${ctx.analysis ? '✓ Complete' : '✗ Missing'}</span></div><div class="flex justify-between border-b border-gray-800 pb-1"><span class="text-gray-500">Strategy:</span><span class="text-amber-400">${ctx.strategy ? '✓ Generated' : '✗ Missing'}</span></div><div class="flex justify-between border-b border-gray-800 pb-1"><span class="text-gray-500">Audit:</span><span class="text-red-400">${ctx.audit ? '✓ Audited' : '✗ Missing'}</span></div><div class="flex justify-between"><span class="text-gray-500">Deliverables:</span><span class="text-green-400">${ctx.deliverables && ctx.deliverables.total_artifacts ? ctx.deliverables.total_artifacts + ' artifacts' : '✗ Blocked'}</span></div></div>`;
                ...
                if (response.deliverables && response.deliverables.status !== 'blocked') {
                    ...
                    artifacts.forEach(art => {
                        ...
                        if (data && data.status === 'success') {
                            artifactsGrid.innerHTML += `<div class="bg-[#07080c] border border-${art.color}-900/50 rounded-lg p-3 text-center"><div class="text-2xl mb-2">${art.icon}</div><div class="text-xs font-mono text-${art.color}-400 uppercase">${art.name}</div><div class="text-[10px] text-gray-500 mt-1">${data.format.toUpperCase()}</div></div>`;
                        }
                    });
                }
                ...
                document.getElementById('artifactCount').textContent = (response.deliverables && response.deliverables.total_artifacts) || 0;
