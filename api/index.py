import logging
from band_bridge import BandChatBridge, BAND_API_KEY, BAND_ROOM_ID

# Check if Band chat backend is available
BAND_BACKEND_AVAILABLE = bool(BAND_API_KEY and BAND_ROOM_ID)
if BAND_BACKEND_AVAILABLE:
    logging.info("[ShadowSignal] Band chat backend connected")
else:
    logging.warning("[ShadowSignal] Band chat backend not configured, using internal workflow")
...
def run_band_workflow(target_competitor):
    """Run workflow via Band chat backend agents, with internal fallback."""

    # Try Band chat backend first
    if BAND_BACKEND_AVAILABLE:
        try:
            logging.info(f"[ShadowSignal] Triggering Band chat workflow for {target_competitor}")
            bridge = BandChatBridge()
            workflow_id = bridge.trigger_workflow(target_competitor)
            results = bridge.poll_workflow(workflow_id, timeout=120)

            if results.get("status") in ["complete", "blocked"]:
                logging.info(f"[ShadowSignal] Band workflow completed: {results['status']}")
                return _format_bridge_results(results, target_competitor, workflow_id)
        except Exception as e:
            logging.error(f"[ShadowSignal] Band backend failed: {e}, falling back to internal")

    # Fallback to internal workflow
    logging.info(f"[ShadowSignal] Running internal workflow for {target_competitor}")
    return _run_internal_workflow(target_competitor)


def _format_bridge_results(results, target, workflow_id):
    """Format Band bridge results for frontend."""
    ...
    try:
        analysis_json = json.loads(analysis) if isinstance(analysis, str) and analysis.strip().startswith("{") else {"raw_text": analysis}
    except:
        analysis_json = {"raw_text": analysis}

    try:
        strategy_json = json.loads(strategy) if isinstance(strategy, str) and strategy.strip().startswith("[") else [{"raw_text": strategy}]
    except:
        strategy_json = [{"raw_text": strategy}]

    interactions = len([e for e in results.get("ledger", []) 
                       if any(x in e.get("action", "") for x in ["ASK", "RESPONSE", "CHALLENGE"])])
    escalations = [e for e in results.get("ledger", []) if "ESCALATION" in e.get("action", "")]
    is_blocked = "[CRITICAL RISK]" in audit
    ...
    return {
        ...
        "shared_context": {
            "raw_intel": results.get("raw_intel"),
            "analysis": analysis_json,
            "strategy": strategy_json,
            "audit": audit,
            "deliverables": {
                "total_artifacts": 0 if is_blocked else 3,
                "chart": {"status": "blocked" if is_blocked else "success"},
                "slide_deck": {"status": "blocked" if is_blocked else "success"},
                "roi_calculator": {"status": "blocked" if is_blocked else "success"}
            }
        },
        ...
        "deliverables": {
            "status": "blocked" if is_blocked else "success",
            "total_artifacts": 0 if is_blocked else 3,
            "chart": {"status": "blocked" if is_blocked else "success", "format": "png"},
            "slide_deck": {"status": "blocked" if is_blocked else "success", "format": "pptx"},
            "roi_calculator": {"status": "blocked" if is_blocked else "success", "format": "python"}
        }
    }


def _run_internal_workflow(target_competitor):
    """Original internal workflow (fallback)."""
