    ...
    try:
        ...
        if Config.BAND_API_KEY and Config.BAND_ROOM_ID:
            try:
                ...
                result = bridge.poll_workflow(workflow_id, timeout=300)

                # If bridge returns timeout, agents are still processing - return processing status
                if result.get("status") == "timeout":
                    logger.info("Band bridge timeout - agents still processing for: %s", target)
                    return jsonify({
                        "target": target,
                        "timestamp": datetime.utcnow().isoformat(),
                        "status": "processing",
                        "message": "Agents are analyzing. Results will appear in the Band chat room.",
        ...
        # FALLBACK: Only return demo data if Band is not configured at all
...
def search():
    try:
        ...
        try:
            ...
            fd = BrightDataClient()
            result = fd.search_google(query, num_results=5)
            return jsonify(result)
        except Exception as e:
            logger.error("Free data search error: %s", e)
            return jsonify({
                "success": False,
                "error": str(e),
                "results": [],
...
<div class="boot-line"><span class="boot-prompt">$</span> connecting to Free Data API (RSS + DuckDuckGo)...</div>
...
<span style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:#64748b">Payment Stream</span>
<span style="font-family:monospace;font-size:12px;color:#2775ca" id="main-tx-count">0 transactions</span>
</div>
<div id="main-stream-list" style="max-height:300px;overflow-y:auto;display:flex;flex-direction:column;gap:6px">
<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:120px;color:#64748b;gap:8px;font-size:12px">
<div style="font-size:24px">⚡</div>
<p>Waiting for agent activity...</p>
</div>
</div>
<div style="margin-top:12px;padding-top:12px;border-top:1px solid #1a2332">
<div style="display:flex;justify-content:space-between;align-items:center">
<span style="font-size:11px;color:#64748b">Total Spent</span>
<span style="font-family:monospace;font-size:16px;font-weight:600;color:#2775ca" id="main-total-paid">0.000</span>
</div>
<div style="display:flex;justify-content:space-between;align-items:center;margin-top:4px">
<span style="font-size:11px;color:#64748b">Active Agents</span>
<span style="font-family:monospace;font-size:14px;color:#00d4aa" id="main-active-agents">0</span>
</div>
</div>
</div>
</div>
...
<span class="status-item" id="api-status"><span class="status-indicator-dot offline"></span>FREE DATA API</span>
...
this._log('info','⏳ Agents are analyzing in the background. Check the Band chat room for live updates.');
this._setAgentStatus('investigator','processing');
setTimeout(()=>{this.isAnalyzing=false;btn.disabled=false;btn.innerHTML='<span class="btn-text">EXECUTE INTELLIGENCE GATHERING</span>';},5000);
return;
}
if(result){
...
this.isAnalyzing=false;
btn.disabled=false;
btn.innerHTML='<span class="btn-text">EXECUTE INTELLIGENCE GATHERING</span>';
}
...
try{
this._setAgentStatus('investigator','processing');
this._log('info','[Investigator] Connecting to Free Data API (RSS + DuckDuckGo)...');
...
// === REAL-TIME PAYMENT STREAM (integrated into main UI) ===
(function(){
  const AGENTS=[
  ...
  let txs=[],spent=0,asp={},aset=new Set(),ctr=0;

  function renderMain(){
    const list=document.getElementById("main-stream-list");
    if(!list)return;
    if(!txs.length){list.innerHTML='<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:120px;color:#64748b;gap:8px;font-size:12px"><div style="font-size:24px">⚡</div><p>Waiting for agent activity...</p></div>';return;}
    list.innerHTML=txs.slice(0,20).map((tx,i)=>`<div style="background:#0d1117;border:1px solid #1a2332;border-radius:8px;padding:10px 12px;display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;${i===0?'border-color:rgba(39,117,202,0.5);background:rgba(39,117,202,0.05)':''}">
<div style="width:32px;height:32px;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:14px;background:${tx.c}22;border:1px solid ${tx.c}44">${tx.e}</div>
<div><div style="font-size:12px;font-weight:500">ShadowSignal ${tx.n}</div><div style="display:flex;align-items:center;gap:6px"><span style="font-size:10px;color:#64748b;font-family:monospace">${tx.ac}</span><span style="font-size:10px;color:#334155;font-family:monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:80px">${tx.h}</span></div>
<span style="display:inline-flex;align-items:center;gap:3px;font-size:10px;font-family:monospace;padding:2px 6px;border-radius:4px;margin-top:2px;${tx.s==='confirmed'?'background:rgba(0,212,170,0.1);color:#00d4aa;border:1px solid rgba(0,212,170,0.2)':tx.s==='failed'?'background:rgba(239,68,68,0.1);color:#ef4444;border:1px solid rgba(239,68,68,0.2)':'background:rgba(245,158,11,0.1);color:#f59e0b;border:1px solid rgba(245,158,11,0.2)'}">${tx.s==='confirmed'?'✓ confirmed':tx.s==='failed'?'✗ failed':'◌ pending'}</span></div>
<div style="text-align:right"><div style="font-family:monospace;font-size:13px;font-weight:600;color:#2775ca">−${tx.amt.toFixed(3)}</div><div style="font-size:10px;color:#64748b">${tx.t}</div></div>
</div>`).join("");
  }

  function updateStats(){
    const tc=document.getElementById("main-tx-count");
    const tp=document.getElementById("main-total-paid");
    const aa=document.getElementById("main-active-agents");
    if(tc)tc.textContent=txs.length+" transaction"+(txs.length!==1?"s":"");
    if(tp)tp.textContent=spent.toFixed(3);
    if(aa)aa.textContent=aset.size;
  }

  const es=new EventSource('/api/pay/stream');
  es.onmessage=(e)=>{
    try{
      const d=JSON.parse(e.data);
      if(d.type==='tx'){
        const tx={id:++ctr,n:d.n,ac:d.ac,amt:d.amt,e:d.e,c:d.c,h:d.h,t:d.t,s:d.s,r:d.r||''};
        txs.unshift(tx);
        if(d.s!=='failed'){spent+=tx.amt;asp[d.n]=(asp[d.n]||0)+tx.amt;}
        aset.add(d.n);
        renderMain();updateStats();
        if(d.s!=='failed'){setTimeout(()=>{tx.s='confirmed';renderMain()},1500);}
      }
    }catch(err){}
  };
  es.onerror=()=>{console.log('Main UI SSE reconnecting...')};
})();
</script>
</body>
</html>"""
...
# ==================== SSE PAYMENT STREAM ====================
@app.route("/api/pay/stream")
def pay_stream():
    """SSE endpoint that streams real payment events from agent network."""
    def event_stream():
        import json
        import time
        import os

        log_path = os.getenv("DATA_DIR", "/tmp") + "/payment_events.jsonl"
        last_pos = 0
        sent_ids = set()

        # Send initial connection event
        yield "data: " + json.dumps({"type": "connected", "message": "Payment stream active"}) + "\n\n"
                    ...
                    with open(log_path, "r") as f:
                        ...
                        for line in lines:
                            ...
                            try:
                                ...
                                if tx_id and tx_id not in sent_ids:
                                    sent_ids.add(tx_id)
                                    # Normalize to frontend format
                                    agent_map = {
                                        "ShadowSignal Investigator": {"n": "Investigator", "a": "web_scrape", "e": "🔍", "c": "#7c3aed", "amt": 0.005},
                                        "ShadowSignal Analyst": {"n": "Analyst", "a": "analysis", "e": "📊", "c": "#2775ca", "amt": 0.003},
                                        "ShadowSignal Strategist": {"n": "Strategist", "a": "strategy", "e": "♟️", "c": "#0891b2", "amt": 0.003},
                                        "ShadowSignal Regulatory": {"n": "Regulatory", "a": "compliance", "e": "⚖️", "c": "#059669", "amt": 0.002},
                                        "ShadowSignal Codeband": {"n": "Codeband", "a": "report", "e": "📋", "c": "#d97706", "amt": 0.001},
                                    }
                                    agent_name = event.get("agent", "")
                                    info = agent_map.get(agent_name, {"n": "Agent", "a": "llm_call", "e": "🤖", "c": "#64748b", "amt": 0.001})

                                    payload = {
                                        "type": "tx",
                                        "id": tx_id,
                                        "n": info["n"],
                                        "ac": event.get("action", info["a"]),
                                        "amt": float(event.get("amount", info["amt"])),
                                        "e": info["e"],
                                        "c": info["c"],
                                        "h": tx_id[:10] if tx_id else "0x" + "0"*8,
                                        "t": event.get("timestamp", "").split("T")[1].split(".")[0] if "T" in event.get("timestamp", "") else "--:--:--",
                                        "s": event.get("status", "confirmed"),
                                        "r": event.get("reason", ""),
                                    }
...
<span class="ts ${tx.s==='confirmed'?'sc':'sp2'}">${tx.s==='confirmed'?'✓ confirmed':tx.s==='failed'?'✗ failed':'◌ pending'}</span></div>
...
// Connect to real-time SSE payment stream
const es=new EventSource('/api/pay/stream');
es.onmessage=(e)=>{
  try{
    const d=JSON.parse(e.data);
    if(d.type==='tx'){
      const tx={id:++ctr,n:d.n,ac:d.ac,amt:d.amt,e:d.e,c:d.c,h:d.h,t:d.t,s:d.s};
      txs.unshift(tx);spent+=tx.amt;asp[d.n]=(asp[d.n]||0)+tx.amt;aset.add(d.n);
      render();stats();
      setTimeout(()=>{tx.s='confirmed';render()},1500);
    }
  }catch(err){}
};
